INSERT INTO results 
select "D'Hondt (region)" as system, 
        ...
        (select party_name from party_seats where system = "dh_region" order by seats desc limit 1) as winning_party,
        max(ps.seats) over() - ps.seats as seat_difference_from_winner
    from parties p
        join party_seats ps on ps.party_name = p.party_name
        where ps.system = "dh_region"
        and ps.seats > 0
