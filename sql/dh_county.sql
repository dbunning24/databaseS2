INSERT INTO results 
select "D'Hondt (county)" as system, 
        ...
        (select party_name from party_seats where system = "dh_county" order by seats desc limit 1) as [winning party],
        max(ps.seats) over() - ps.seats as [seat difference from winner]
    from parties p
        join party_seats ps on ps.party_name = p.party_name
        where ps.system = "dh_county"
        and ps.seats > 0
